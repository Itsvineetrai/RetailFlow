    ...
    def get_session(
        ...
        if cls._spark is None:
            ...
            if running_locally:
                ...
                logger.info(
                    "Detected local Windows environment."
                )
                ...
                logger.info(
                    f"Connecting to Spark Master: {master_url}"
                )

            # ---------------------------------------------------------
            # Create Session
            # ---------------------------------------------------------
            raw_endpoint = str(settings.minio_endpoint).replace("http://", "").replace("https://", "")
            if "localhost" in raw_endpoint.lower():
                spark_endpoint = "minio:9000"
            else:
                spark_endpoint = raw_endpoint
